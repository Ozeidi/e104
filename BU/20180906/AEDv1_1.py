...
class AE(Disaggregator):
    ...
    def _create_train_set(self):
        ...
        train_buidlings = self.meta['train_buildings']
        all_x = np.zeros((1,self.sequence_length,1))
        all_y = np.zeros((1,self.sequence_length,1))
        ...
        for i, building_no in enumerate(train_buidlings):
            ...
            # size = chunk.shape[0]
            # additional = self.sequence_length - (size % self.sequence_length)
            # add_df = pd.DataFrame(np.zeros((additional, 2)))
            # chunk = chunk.append(add_df,  ignore_index=True)
            logger.info('Start batch generation')
            X_batch[i], Y_batch[i] = self.gen_batch(mains_s, meter_s, chunk.shape[0]-self.sequence_length, 0, self.sequence_length)
    ...
    def train_across_buildings_chunk(self, mainchunks, meterchunks,synthetic_source):
        ...
        X_batch, Y_batch = self.gen_batch(X_batch, Y_batch, X_batch.shape[0]-self.sequence_length, 5, self.sequence_length)
