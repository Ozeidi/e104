...
from tensorflow.python.keras.layers import Dense, Conv1D, GRU, Bidirectional, Dropout, Flatten, Reshape
...
class AE(Disaggregator):
    ...
    def train_across_buildings(self, mainlist, epochs=1, batch_size=128, **load_kwargs):
        ...
        **load_kwargs : keyword arguments passed to `meter.power_series()`
        ...
        while(run):
            ...
            self.train_across_buildings_chunk(mainchunks, epochs, batch_size)
    ...
    def train_across_buildings_chunk(self, mainchunks, epochs, batch_size):
        ...
        # Train model
        self.model.fit(X_batch, X_batch, batch_size= batch_size,epochs= epochs,shuffle=True)
        print("\n")
    ...
    def _create_model(self):
        '''Creates the Auto encoder module described in the NEURALNILM  paper
        '''
        model = Sequential()

        # 1D Conv
        model.add(Conv1D(8, 4, activation="linear", input_shape=(self.window_size, 1), padding="same", strides=1))
        model.add(Flatten())

        # Fully Connected Layers
        model.add(Dropout(0.2))
        model.add(Dense((self.window_size-0)*8, activation='relu'))
        ...
        # 1D Conv
        model.add(Reshape(((self.window_size-0), 8)))
